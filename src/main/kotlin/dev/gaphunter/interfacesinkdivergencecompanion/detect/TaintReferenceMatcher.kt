...
/** Whether [expression] is a BARE reference to [targetName], or a `+` concatenation where at least one operand is -- same "direct reference or one-hop concatenation" shape this catalog's other sink finders use. Any wrapping method call breaks the chain. */
object TaintReferenceMatcher {
    ...
    fun isTaintedReference(expression: PsiExpression, targetNames: Set<String>): Boolean =
        targetNames.any { isTaintedReference(expression, it) }
}
